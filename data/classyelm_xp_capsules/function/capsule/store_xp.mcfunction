...
execute if entity @s[tag=cxpcv1tscxp,scores={StoreXP=..-1}] unless score @s cxpcv1oxpa >= @s cxpcv1oxpco run tellraw @s [{"text":"You cannot store ","color":"yellow"},{"score":{"name":"@s","objective":"cxpcv1oxpco"}}," XP because you only have ",{"score":{"name":"@s","objective":"cxpcv1oxpa"}}," XP available."]
